...
:::
3. Запустите локальный сервер для предпросмотра документации: в секции `NPM SCRIPTS` нажмите на кнопку запуска на строчке `dev`. Откроется терминал, и после окончания запуска в нём будет ссылка на локальный сервер (обычно [http://localhost:5173/](http://localhost:5173/)). Перейдите по этой ссылке в браузере.
