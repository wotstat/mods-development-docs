...
as3/bin
```
Этот файл указывает Git, какие файлы и папки не нужно отслеживать. Мы исключаем скомпилированные файлы, архивы модов и папки с исходным кодом игры и скомпилированными `AS3`‑файлами.
...
```
> Ссылку на репозиторий можно найти на его странице; она будет выглядеть примерно так: `https://github.com/SoprachevAK/my-first-mod.git`.
...
Команды в файле должны соответствовать тем, что в `build.bat` для сборки `AS3`‑части. Если несколько `SWF`‑файлов — добавьте команды `mxmlc` для каждого.
...
Добавьте описание изменений и уберите галочку `This is a pre-release`, если хотите сделать релиз публичным. Затем нажмите `Update release`.
