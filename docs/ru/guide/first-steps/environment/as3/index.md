...
- [Apache Royale](https://royale.apache.org/download/) – компилятор `AS3` в `SWF`. Нужно скачать архив `APACHE ROYALE JS/SWF` с [официального сайта](https://royale.apache.org/download/) и распаковать его в удобное место на диске, например `C:\apache-royale`.
...
- `libs` – здесь будут находиться сторонние библиотеки, которые понадобятся для компиляции
- `bin` – здесь будет находиться скомпилированный файл `SWF`

Также скачайте конфигурационный файл [`asconfig.json`](/download/mod-build/asconfig.json){download}, в котором указаны настройки для расширения, и [`build-config.xml`](/download/mod-build/build-config.xml){download} с настройками компиляции `SWF`.
...
В дополнение к игровым библиотекам вам необходима ещё основная библиотека `playerglobal.swc`. [Скачайте](/download/playerglobal.swc){target="_blank"} её и поместите в папку `as3/libs/`.

> Если вы добавляете `damageIndicator.swc`, `directionIndicator.swc` или `predictionIndicator.swc`, не забудьте добавить их в конфигурационные файлы `build-config.xml` и `asconfig.json` по аналогии с другими библиотеками.
...
На 4-й строке укажите путь к папке, в которую распаковали `Apache Royale`.
...
- `src/my/first_mod/HelloWorldWindow.as` – файл с исходным кодом, с которого начинается компиляция (точка входа)
...
Обновление основного скрипта не требуется, если вы настроили по инструкции [окружение для Python](../python/), то можете убедиться, что там есть блок, который запускает `as3/build.bat`, если он присутствует в проекте:
...
> Если расширение `ActionScript & MXML` не работает и вызывает ошибку `as3mxml.java.path in settings does not point to a valid executable`, то необходимо проверить правильность установки `Java JDK`, и убедиться, что он указан в `PATH`.
...
- Собранный `SWF` можно тестировать без переупаковки мода и перезапуска игры: поместите его в `res_mods`, а после сборки пересоздавайте окно на следующем кадре. Подробнее — в разделе [«Тестирование SWF без перезапуска игры»](../../../scripting/as3-theory/#hot-reload).
