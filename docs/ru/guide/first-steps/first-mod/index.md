...
Первое, что приходит в голову, — добавить кнопку «Демонтаж» рядом с «Установить» в окне установки оборудования. Однако интерфейс выбора оборудования сделан на `GF`, который крайне тяжело модифицируется.

::: details Как понять на чём сделан интерфейс
Отличить GF‑интерфейс можно просто: в файле `res/packages/gui-part.pkg/gui/gameface/styles/default.css` добавьте в конце стиль:
```css [default.css]
* {
...
Он добавит зелёную рамку ко всем элементам GF‑интерфейса, что позволит увидеть, какие элементы к нему относятся.
...
В результате видно, что весь интерфейс демонтажа — это GF.

![gf-border](./assets/gf-interface.webp)
:::
...
В игре демонтаж происходит через `GF`‑кнопку. Найти, что она вызывает, непросто. Можно попробовать поиск по `demount`, однако он даёт много лишнего.
